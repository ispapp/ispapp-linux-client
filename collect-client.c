...
char *
escape_string_for_json (char *str)
{
...
int
l_strcpy (char *dest, char *src, int start, int end)
{
...
int
wss_frame_encode_message (char *output_buf, int type, char *buf)
{
...
char *
dns_lookup (char *addr_host, struct sockaddr_in *addr_con)
{
  // printf("\nResolving DNS..\n");
...
void
send_ping (struct sockaddr_in *ping_addr, char *ping_dom, char *ping_ip,
	   char *hostname_str, struct ping_response *pr)
{
  ...
  while (sent < 5)
      ...
      for (i = 0; i < sizeof (pckt.msg) - 1; i++)
	{
      ...
      if (sendto
	  (sockfd, &pckt, sizeof (pckt), 0, (struct sockaddr *) ping_addr,
	   sizeof (*ping_addr)) <= 0)
	{
      ...
      if (recvfrom
	  (sockfd, &pckt, sizeof (pckt), 0, (struct sockaddr *) &r_addr,
	   (socklen_t *) & addr_len) <= 0 && msg_count > 1)
	{
      ...
      else
	{
	  ...
	  rtt_msec =
	    (double) (time_end.tv_sec - time_start.tv_sec) * 1000.0 +
	    timeElapsed;
...
initNl80211 (Netlink * nl)
{
  ...
  nl_cb_set (nl->cb1, NL_CB_FINISH, NL_CB_CUSTOM, finish_handler,
	     &(nl->result1));
  nl_cb_set (nl->cb2, NL_CB_VALID, NL_CB_CUSTOM, getWifiInfo_callback, 0);
  nl_cb_set (nl->cb2, NL_CB_FINISH, NL_CB_CUSTOM, finish_handler,
	     &(nl->result2));
...
static int
getWifiName_callback (struct nl_msg *msg, void *arg)
{
  ...
  all_wifi_indexes[wifi_index_count] =
    nla_get_u32 (tb_msg[NL80211_ATTR_IFINDEX]);
  wifi_index_count++;
...
static int
getWifiInfo_callback (struct nl_msg *msg, void *arg)
{
  ...
  while (c < arraylen)
      ...
      if (strcmp (json_object_get_string (interface), dev) == 0)
	{
	  ...
	  json_object_object_add (station, "mac",
				  json_object_new_string (mac_addr));
	  ...
	  json_object_object_add (station, "sentBytes",
				  json_object_new_int64 (nla_get_u64
							 (sinfo
							  [NL80211_STA_INFO_TX_BYTES64])));
	  json_object_object_add (station, "recBytes",
				  json_object_new_int64 (nla_get_u64
							 (sinfo
...
getWifiStatus (Netlink * nl)
{
  ...
  genlmsg_put (msg1,
	       NL_AUTO_PORT,
	       NL_AUTO_SEQ,
	       nl->id, 0, NLM_F_DUMP, NL80211_CMD_GET_INTERFACE, 0);
  ...
  while (c < wifi_index_count)
      ...
      if (!msg2)
	{
      ...
      genlmsg_put (msg2,
		   NL_AUTO_PORT,
		   NL_AUTO_SEQ,
		   nl->id, 0, NLM_F_DUMP, NL80211_CMD_GET_STATION, 0);
...
sendLoop (void *input)
{
  ...
  while (1)
      ...
      while (1)
	{
	  ...
	  printf
	    ("start: %u, now: %u, update_wait: %i; sending update in %u seconds.\n",
      ...
      int socket_poll =
	mbedtls_net_poll (&server_fd, MBEDTLS_NET_POLL_WRITE, 0);
	  ...
	  for (ifa = ifaddr; ifa != NULL; ifa = ifa->ifa_next)
	      ...
	      if (family == AF_PACKET && ifa->ifa_data != NULL)
		{
		  ...
		  if (real_iface_count == 0)
			...
			realloc (interface_json_string,
				 strlen (interface_json_string) + 800);
	...
	system_info.totalram * system_info.mem_unit;
      unsigned long long freeram = system_info.freeram * system_info.mem_unit;
      unsigned long long bufferram =
	system_info.bufferram * system_info.mem_unit;
      unsigned long long sharedram =
	system_info.sharedram * system_info.mem_unit;
	  ...
	  if (strlen (disks_json_string) == 0)
	    {
	      ...
	      sprintf (disks_json_string,
		       "[{\"mount\": \"%s\", \"used\": %llu, \"avail\": %llu}",
		       m->mnt_dir, used, avail);
	  ...
	  else
	    {
	      ...
	      disks_json_string =
		realloc (disks_json_string, strlen (disks_json_string) + 200);
	      // create temporary string
      ...
      else
	{
      ...
      char *system_json_string =
	calloc (strlen (disks_json_string) + 400, sizeof (char));

      sprintf (system_json_string,
	       "{\"load\": {\"one\": %ld, \"five\": %ld, \"fifteen\": %ld, \"processCount\": %llu}, \"memory\": {\"total\": %llu, \"free\": %llu, \"buffers\": %llu, \"cache\": %llu}, \"disks\": %s}",
	       system_info.loads[0], system_info.loads[1],
	       system_info.loads[2], procs, totalram, freeram, bufferram,
	       sharedram, disks_json_string);
      ...
      else
	{
      ...
      char *updateString =
	calloc (600 + strlen (wan_ip) + +strlen (wap_json_string) +
		strlen (interface_json_string) + strlen (disks_json_string) +
		strlen (system_json_string), sizeof (char));
      sprintf (updateString,
	       "{\"type\": \"update\", \"uptime\": %llu, \"wanIp\": \"%s\", \"collectors\": {\"wap\": %s, \"ping\": [{\"host\": \"%s\", \"avgRtt\": %lf, \"minRtt\": %lf, \"maxRtt\": %lf, \"loss\": %d}], \"system\": %s, \"interface\": %s, \"disks\": %s}}",
	       uptime, wan_ip, wap_json_string, pr.host, pr.avgRtt, pr.minRtt,
	       pr.maxRtt, pr.loss, system_json_string, interface_json_string,
	       disks_json_string);
      ...
      long long unsigned int sbuf_len =
	wss_frame_encode_message (sbuf, 1, updateString);
...
int
popenTHREE (int *threepipe, const char *command)
{
...
int
pcloseTHREE (int pid, int *threepipe)
{
  int status;
  close (threepipe[0]);
  close (threepipe[1]);
...
main (int argc, char **argv)
{
  ...
  if (argc != 15)
	...
	("\tADDRESS:\tdev.ispapp.co\t\t(the address of the websocket server)\n");
      ...
      printf
	("\tWLAN_IF:\teth0\t\t\t(login field is set to the mac address of WLAN_IF)\n");
  ...
  while (1)
      ...
      else
	{
      ...
      size_t b64len;
      int b64_encode_status =
	mbedtls_base64_encode (randomB64String, sizeof randomB64String,
			       &b64len, randomBytes, 16);
      ...
      int sha1_hash_status =
	mbedtls_sha1_ret (hashCheckString, strlen (hashCheckString),
			  sha1HashCheckString);
      ...
      int b64_hash_check_encode_status =
	mbedtls_base64_encode (b64HashCheckString, sizeof b64HashCheckString,
			       &b64HashCheckStringLen, sha1HashCheckString,
			       sizeof sha1HashCheckString);
      ...
      if ((ret =
	   mbedtls_net_connect (&server_fd, root_address, root_port,
      ...
      else
	{
      ...
      while ((ret =
	      mbedtls_ssl_write (&ssl, reqString, strlen (reqString))) <= 0)
	{
	  if (ret != MBEDTLS_ERR_SSL_WANT_READ
	      && ret != MBEDTLS_ERR_SSL_WANT_WRITE)
	    {
      ...
      do
	{
	  ...
	  if (first_response == 0)
	      ...
	      while (c < len)
		{
		  ...
		  if (buf[c] == '\n' && buf[c - 1] == '\r')
		      ...
		      if (strstr (hbuf, "HTTP/1.1 ") != 0)
			{
			  ...
			  if (headerHttp[0] == '1' && headerHttp[1] == '0'
			      && headerHttp[2] == '1')
			    {
			      // 101 response code
			      validHeaderCount++;
		...
		{
		  sprintf (os_version, "%s %s %s %s %s", uts.sysname,
			   uts.nodename, uts.release, uts.version,
			   uts.machine);
		}

	      char *config_req =
		calloc (strlen (root_mac) + strlen (root_collect_key) +
			strlen (root_client_info) + strlen (os_version) +
			strlen (os_version) + strlen (root_hardware_make) +
			strlen (root_hardware_model) +
	      ...
	      long long unsigned int sbuf_len =
		wss_frame_encode_message (sbuf, 1, config_req);

	      if (sbuf_len >= 0)
		{

		  while ((ret =
			  mbedtls_ssl_write (&ssl, sbuf, sbuf_len)) <= 0)
		    {
		      if (ret != MBEDTLS_ERR_SSL_WANT_READ
			  && ret != MBEDTLS_ERR_SSL_WANT_WRITE)
			{
	      ...
	      else if (buf[1] == 127)
		{
		  ...
		  payload_len =
		    (buf[2] << 8) + buf[3] + buf[4] + buf[5] + buf[6] +
		    buf[7] + buf[8] + buf[9];
		  payload_start = 10;
	      ...
	      else
		{
		  ...
		  if (strcmp (type_string, "error") == 0)
		  ...
		  else if (authed_flag == 0
			   && strcmp (type_string, "config") == 0)
		    {
		      ...
		      if (json_object_object_get_ex (json, "client", &client))
			{
			  ...
			  if (json_object_object_get_ex
			    ...
			    {
			      bool authed_bool =
				json_object_get_boolean (authed);

			      if (authed_bool)
				{
				  printf
				    ("client.authed is true, this host is authenticated and it is time to set startup configuration values\n");
				  authed_flag = 1;

				  // start a detached thread for sending updates
				  pthread_create (&thread_id, NULL, sendLoop,
						  NULL);
				  ...
				  if (root_config_file[0] != '\0')
				    {
				      FILE *fp =
					fopen (root_config_file, "w");
				      if (!fp)
					{
					  printf
					    ("There was an error opening '%s', could not write the configuration to file.\n",
					     root_config_file);
					  exit (0);
					}
				      else
		  ...
		  else if (authed_flag == 1
			   && strcmp (type_string, "update") == 0)
		    {
		      ...
		      while (1)
			{
			  ...
			  if (ch != EOF)
			      ...
			      if (c > current_size)
				{
				  ...
				  out_stdout =
				    realloc (out_stdout, current_size);
				}
		      ...
		      if (timed_out == 1)
			{
			  ...
			  sprintf (out_stderr,
				   "process timeout of 4 seconds reached");
				  ...
				  if (c > current_size)
				    {
				      current_size += PATH_MAX;
				      out_stderr =
					realloc (out_stderr, current_size);
				    }
		      ...
		      if (strlen (out_stdout) > 4000)
			{
			  ...
			  sprintf (out_stderr,
				   "command output is too long: %u bytes\ntry sending the output to a file with >",
				   strlen (out_stdout));
			  sprintf (out_stdout, "");
		      ...
		      char *e_out_stdout =
			calloc ((strlen (out_stdout) * 2) + 1, sizeof (char));
		      size_t e_out_stdout_len;
		      int e_out_stdout_encode_status =
			mbedtls_base64_encode (e_out_stdout,
					       strlen (out_stdout) * 2,
					       &e_out_stdout_len, out_stdout,
					       strlen (out_stdout));
		      ...
		      // allocate enough space for the b64 encoded string by using twice the strlen, +1 incase the strlen is 0
		      char *e_out_stderr =
			calloc ((strlen (out_stderr) * 2) + 1, sizeof (char));
		      size_t e_out_stderr_len;
		      int e_out_stderr_encode_status =
			mbedtls_base64_encode (e_out_stderr,
					       strlen (out_stderr) * 2,
					       &e_out_stderr_len, out_stderr,
					       strlen (out_stderr));
		      ...
		      const char *uuidv4_string =
			json_object_get_string (uuidv4);

		      struct json_object *ws_id;
		      json_object_object_get_ex (json, "ws_id", &ws_id);
		      const char *ws_id_string =
			json_object_get_string (ws_id);

		      char *update = calloc (strlen (uuidv4_string) +
					     strlen (e_out_stdout) +
					     strlen (e_out_stderr) +
					     strlen (ws_id_string) + 200,
					     sizeof (char));
		      sprintf (update,
			       "{\"type\": \"cmd\", \"uuidv4\": \"%s\", \"stdout\": \"%s\", \"stderr\": \"%s\", \"ws_id\": \"%s\"}",
			       uuidv4_string, e_out_stdout, e_out_stderr,
			       ws_id_string);
		      ...
		      char *sbuf =
			calloc (strlen (update) + 14, sizeof (char));
		      long long unsigned int sbuf_len =
			wss_frame_encode_message (sbuf, 1, update);

		      if (sbuf_len >= 0)
			{

			  while ((ret =
				  mbedtls_ssl_write (&ssl, sbuf,
						     sbuf_len)) <= 0)
			    {
			      if (ret != MBEDTLS_ERR_SSL_WANT_READ
				  && ret != MBEDTLS_ERR_SSL_WANT_WRITE)
